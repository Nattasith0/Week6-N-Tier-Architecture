...
test_endpoint() {
    ...
    if [[ "$url" == https://* ]]; then
        response=$(curl -sk -X POST -H "Content-Type: application/json" -d "$data" "$url")
    else
    ...
    if [[ "$url" == https://* ]]; then
        response=$(curl -sk -X PATCH -H "Content-Type: application/json" -d "$data" "$url")
    else
